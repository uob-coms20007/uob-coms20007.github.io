...
4. Using (P3), we can pump the $v$ part to obtain other words in this language.  So we conclude that, e.g. $$uv^2w = a^{p!+\lvert v \rvert} \in \{a^{n!} \mid n \in \mathbb{N}\}$$.  But this is nonsense, because $$p!+\lvert v \rvert$$ cannot have the form $n!$ for any natural number $n$.  To see why, we observe that $$p! + \lvert v \rvert$$ must lie strictly between $p!$ and $(p+1)!$ (using our knowledge that $p \geq 1$ and $1 \leq \lvert v \rvert p$):  
