...
It allows us to weaken a strong claim $$\{ p' \}\ S\ \{ q' \}$$, into a more specific claim $$\{ p \}\ S\ \{ q \}$$ that fits with our particular objective.
The _side conditions_ $$p \vDash p'$$ and $$q' \vDash q$$ say that whenever $$p$$ is true $$p'$$ is also true ,and likewise whenever $$q$$ is true $$q'$$ is true (formally, $$p \vDash p'$$ just if $$\llbracket p \rrbracket_\mathcal{B}(\sigma) \Rightarrow \llbracket p' \rrbracket_\mathcal{B}(\sigma)$$ for all stores $$\sigma \in \mathsf{Store}$).
...
For instance, using the rule for assignment, we may conclude that $$\{ x > 0 \}\ x \leftarrow x + 1\ \{ x > 1 \}$$ and we wish to combine this with the fact that $$\{ x \geq 0 \}\ y \leftarrow x\ \{ y \geq 0 \}$$ to derive a Hoare triple for $$x \leftarrow x + 1;\; y \leftarrow y + 1$$.
...
To answer this question, we will work backwards starting with the post-condition $$x \geq 0$$ to try to determine the *weakest* pre-condition that ensures the post-condition is met.
...
The weakest pre-condition is not only a pre-condition that ensures the given post-condition is met, but it is in some sense the canonical such pre-condition and will subsume any other such pre-condition.
For example, $$\mathsf{wp}(x \leftarrow x + 1,\, x > 1)$$ is $$x > 0$$ as it is a valid pre-condition, but also any other pre-condition such as $x > 1$ will be less general.
...
$$
  \mathsf{wp}(S,\, q) = \{ \sigma \in \mathsf{Store} \mid \text{if}\ \langle S,\, \sigma \rangle \rightarrow^* \sigma'\ \text{then}\ \llbracket q \rrbracket_\mathcal{B}(\sigma') \}
$$

### Aside
This correspondence is only intuitive as not all sets of stores can be described by a Boolean expression.
...
The strict definition of the weakest pre-condition, however, only requires that it is the weakest Boolean expression that approximates this set.
However, when a set can not be precisely described by a Boolean expression, we can imagine an ever increase sequence of approximations; and so, in such case the weakest pre-condition would not be defined anyway.
...
So, how are we going to derive the weakest pre-condition for $$x \leftarrow y;\; \mathsf{if}\ x < 0\ \mathsf{then}\ x \leftarrow 0 - x\ \mathsf{else}\ \mathsf{skip}$$ with the post-condition $$x \geq 0$$?
...
At the top-level, our statement is a sequence statement, and thus we will apply the rule for this construct.
To do so, we must find some $p$ and $q$ such that: $$\{ p \}\ x \leftarrow y\ \{ q \}$$ and $$\{ q \}\ \mathsf{if}\ x < 0\ \mathsf{then}\ x \leftarrow 0 - x\ \mathsf{else}\ \mathsf{skip}\ \{ x \geq 0 \}$$ as these are the two premises.

Now we have broken down the problem of finding the weakest pre-condition to this compound statement into two smaller problems that requires us to find the weakest pre-conditions for sub-statements.
