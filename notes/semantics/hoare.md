...
<div class="defn" markdown="1">
  ...
  $$
    \forall \sigma \in \mathsf{Store}.\, \llbracket p \rrbracket_\mathcal{B}(\sigma) = \top \wedge \langle S,\, \sigma \rangle \rightarrow^* \sigma' \Rightarrow \llbracket q \rrbracket_\mathcal{B}(\sigma') 
  $$ 
...
Unlike the other rules we have seen so far, this rule doesn't involve any particular syntatic construct but rather works for an arbitrary statement.
...
Notice we derived this triple without consider why each assumed triples is true or deferring to the operational semantics, so we could have equally done this with some arbitrary statements that perhaps refer to some external function.
